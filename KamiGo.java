 ...
 *
 * <backprop_input_file> is a file we wrote a neural net to
 * 
...
public class KamiGo {
    ...
    /*
     * This gets called first by the main mathod, and initializes necessary classes
     *
     * Also initializes the hash map and other info that we'll need in orer to
     * have pleasant communication between us and Fuego, e.g., because
     * Fuego can use different coordinate systems for indicating moves.
     */
    private void defineStuff(File Kami_Net) {
	// Translate from fuego board notation to ours
	// Utilized in getMoveCoordinates()
    ...
    // Returns whether a not a move was removed
    ...
    // Prints the board state in human readable format
	...
	try {
		...
		Trainer miyagi;
		if (train_from_file) {
		    // Continues training on a given neural net
    ...
    /*
     * A method that checks for player input, then interprates and carries out
     * the given commands. 
     *
     * The current implementation assumes that these commands are written correctly.
     * If you provide incorrect arguments, then the game with throw an error
     * 
     * Please see README.txt for a full description of these commands
     */
	...
	while (true) {
		...
		else if ( user_input[0].equals("quit") )  {
		    break;
		}
    ...
    /*
     * This method was going to be used to communicate with fuego in order to
     * check the legaility of moves and to update boardstate, however this
     * functionality was never completed. 
     *
     * Please disregard this and the similarly named method below.
     */
     ...
     *
     * NOTE: Currently only iterates through the empty locations of the board. There is no
     * call to check if an empty space is legal.
     */
