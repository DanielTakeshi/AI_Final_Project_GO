 ...
 *
 * Called once (by KamiGo) to create the Neural Net through training Much of the
 * functionality is already in GoTree.java We just need to call its "train" method
 * judiciously and pass in the corresponding inputs. Note: this will not be called if the
 * user has specified that we play Go with an ANN that's already been constructed.
 */

public class Trainer {

    /*
     * + transcript holds all transcripts for all games in transcript.txt Transcript for
     * one game is a board state followed by the move. Ex:
     *
    ...
    // inputNum = 42 + 1 bias node, hiddenNum = hidden nodes (not including bias), mew = learn rate
    ...
    /*
     * Constructor for a trainer that only takes in the transcript_files of
     * games. Initializes Go Tree, open transcript file, pre-processes the board states
     * and pro move inputs (a huge time savings!). Then we have lots of info ready for
     * when training actually happens.
     */
	...
	for (int i=0; i<num_moves; i++) {
	    ...
	    // Now we get the length 42 (43 really, due to the bias) feature vector
    ...
    /*
     * Another constructor, which takes in an extra backprop file location. This way, we
     * can continue training from an alraedy-constructed tree. (I.e., "continue where we
     * left off.") Otherwise, it's the same as the other constructor.
     */
	...
	// Why it's different from the previous constructor
    ...
    /*
     * Checks if a given board location is empty. Called by constructor so that we can
     * detect when a new game has begun in the transcript. (Remember, that file consists
     * of almost 200 games!)
     */
    ...
    /*
     * We have a transcript from the constructor; we train once with each game in order,
     * and then repeat the process until we've done it num_repeat times. We use a 'toFile'
     * method so that we can save the results of the training as we go along (for safety
     * reasons, and lets us see the progress of our training).
     */
    ...
    /*
     * Called by runTraining. Given a transcript, feed it to the neural net so it can
     * train, i.e. by calling KamiNet.train with the input, which is the feature vector.
     * To train, we use two moves. One will be the professional's move; the other will be
     * a move chosen at random from the possible empty spots that does not match the pro's
     * move.
     */
	...
	do {
	    x = rand.nextInt(9); // Random int, 0 through 8
	    y = rand.nextInt(9);
    ...
    /*
     * Converts the string of board states from the Transcript file to a 9x9 array. This
     * is called by the constructors since it makes it easier to iterate through the board
     * positions in a meaningful way (9x9 versus a string of 81 charcters) and also lets
     * us visualize the board for debugging purposes.
     */
